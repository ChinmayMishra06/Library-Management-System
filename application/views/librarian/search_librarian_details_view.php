...
?>		
<div class="row">
	<div class="offset-sm-4 col-sm-4" id="librarian_details_form">
		<div class="card">
			<div class="card-header p-0">
				<h4 class="text-center text-white">Librarian Details</h4>
			</div>
			<div class="card-body">
				<?php if($this->session->flashdata('librarian_details_message')): ?>
					<div class="alert alert-<?php if($this->session->flashdata('librarian_details_status')): echo $this->session->flashdata('librarian_details_status'); ?> alert-dismissible"><?= $this->session->flashdata("librarian_details_message"); endif; ?>
				...
				<?= form_open('librarian/librarian_details_check', ["id"=>"librarian_details_by_id_form", "name"=>"librarian_details_by_id_form"]); ?>
					<div class="row">
						<div class="col">
					...
					<div class="row">
						<div class="col">
							<div class="form-group">
								<input type="submit" name="librarian_bdetails_utton" id="librarian_bdetails_utton" class="btn btn-outline-secondary float-right" value="Search">
							</div>
						</div>
					</div>
				<?= form_close(); ?>
				<?= form_open('librarian/librarian_details_check', ["id"=>"librarian_details_by_name_form", "name"=>"librarian_details_by_name_form", "class"=>"d-none"]); ?>
					<div class="row">
						<div class="col">
							...
							<div class="form-group">
								...
								<input type="text" list="librarian_details_name_list" name="librarian_name" id="librarian_name" class="form-control" value="<?php echo set_value('librarian_name'); ?>">
								<datalist id="librarian_details_name_list"></datalist>
							</div>
						</div>
					</div>
					<div class="row">
						<div class="col">
							<div class="form-group">
								<input type="submit" name="search_librarian_details_button" id="search_librarian_details_button" class="btn btn-outline-secondary float-right" value="Search">
							</div>
						</div>
					</div>
				<?= form_close(); ?>
...
<script>
	...
	document.getElementById("details_by").addEventListener('change', function(){
		if(document.getElementById("details_by").value === "details_id"){
			document.getElementById("librarian_details_by_id_form").classList.remove("d-none");
			document.getElementById("librarian_details_by_name_form").classList.add("d-none");
		}
		else{
			document.getElementById("librarian_details_by_id_form").classList.add("d-none");
			document.getElementById("librarian_details_by_name_form").classList.remove("d-none");
		}
	});
	...
	// If user serach librarian details by the id, then librarian_details_by_id_form will be submitted.
	document.getElementById("librarian_details_by_id_form").addEventListener('submit', function(e){
		e.preventDefault();
		
		// This variable is used to store the valiate_form function result.
		var form_submit_result;
		
		// If form_submit_result will be true, then librarian_details_by_id_form will be submitted, else not.			
		form_submit_result = validate_form([librarian_id]);
		if(form_submit_result === true){
			document.getElementById("librarian_details_by_id_form").submit();
		}
	});
	
	// If user serach librarian details by the id, then librarian_details_by_name_form will be submitted.
	document.getElementById("librarian_details_by_name_form").addEventListener('submit', function(e){
		e.preventDefault();

		// This variable is used to store the valiate_form function result.
		var form_submit_result;
		
		// If form_submit_result will be true, then librarian_details_by_name_form will be submitted, else not.			
		form_submit_result = validate_form([librarian_name]);
		if(form_submit_result === true){
			document.getElementById("librarian_details_by_name_form").submit();
		}
	});
	...
	// This will make an ajax request to get librarian name according to keyup event on the search librarian details form's librarian name column.
	document.getElementById('librarian_name').addEventListener('keyup', function(e){
		e.preventDefault();
		
		// This variable is used to store the librarian name, filled on the search librarian details form's librarian name column.
			...
			if(xhr.readyState == 4){
				if((xhr.status >= 200 && xhr.status < 300) || xhr.status == 304){
					if(xhr.responseText){
						...
						document.getElementById('librarian_details_name_list').innerHTML = "";
						...
						if(newResponse.not_found === undefined){
							for(let i=0; i<newResponse.length; i++){
								...
								document.getElementById('librarian_details_name_list').appendChild(new_option);
							}
						}
						else{
