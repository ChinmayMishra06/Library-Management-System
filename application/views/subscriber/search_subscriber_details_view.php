...
?>
<div class="row">
	<div class="offset-sm-4 col-sm-4" id="subscriber_details_form">
		<div class="card">
			<div class="card-header p-0">
				<h4 class="text-center text-white">Subscriber Details</h4>
			</div>
			<div class="card-body">
				<?php if($this->session->flashdata('subscriber_details_message')): ?>
					<div class="alert alert-<?php if($this->session->flashdata('subscriber_details_status')): echo $this->session->flashdata('subscriber_details_status'); ?> alert-dismissible"><?= $this->session->flashdata("subscriber_details_message"); endif; ?>
						<button type="button" class="close" data-dismiss="alert">&times;</button>
					</div>
				<?php endif; ?>
				<div class="row">
					<div class="offset-sm-7 col-sm-5">
						<select class="form-control" id="details_by" name="details_by">
							<option value="details_id">By ID</option>
							<option value="serach_name">By Name</option>
						</select>
					</div>			
				</div>
				<?= form_open('subscriber/subscriber_details_check', ["id"=>"subscriber_details_by_id_form", "name"=>"subscriber_details_by_id_form"]); ?>
					<div class="row">
						<div class="col">
					...
					<div class="row">
						<div class="col">
							<div class="form-group">
								<input type="submit" name="details_subscriber_button" id="details_subscriber_button" class="btn btn-outline-secondary float-right" value="Search">
							</div>
						</div>
					</div>
				<?= form_close(); ?>
				<?= form_open('subscriber/subscriber_details_check', ["id"=>"subscriber_details_by_name_form", "name"=>"subscriber_details_by_name_form", "class"=>"d-none"]); ?>
					<div class="row">
						<div class="col">
							...
							<div class="form-group">
								...
								<input type="text" list="details_subscriber_name_list" name="subscriber_name" id="subscriber_name" class="form-control" value="<?php echo set_value('subscriber_name'); ?>">
								<datalist id="details_subscriber_name_list"></datalist>
							</div>
						</div>
					</div>
					<div class="row">
						<div class="col">
							<div class="form-group">
								<input type="submit" name="subscriber_details_button" id="subscriber_details_button" class="btn btn-outline-secondary float-right" value="Search">
							</div>
						</div>
					</div>
				<?= form_close(); ?>
...
<script>
	...
	document.getElementById("details_by").addEventListener('change', function(){
		if(document.getElementById("details_by").value === "details_id"){
			document.getElementById("subscriber_details_by_id_form").classList.remove("d-none");
			document.getElementById("subscriber_details_by_name_form").classList.add("d-none");
		}
		else{
			document.getElementById("subscriber_details_by_id_form").classList.add("d-none");
			document.getElementById("subscriber_details_by_name_form").classList.remove("d-none");
		}
	});
	...
	// If user search subscriber by id, then subscriber_details_by_id_form will be submitted.
	document.getElementById("subscriber_details_by_id_form").addEventListener('submit', function(e){
		e.preventDefault();

		// This variable is used to store the validate_form function result.
		var form_submit_result;
		
		// If form_submit_result will be true, then subscriber_details_by_id_form will be submitted, else not.
		form_submit_result = validate_form([subscriber_id]);
		if(form_submit_result === true){
			document.getElementById("subscriber_details_by_id_form").submit();
		}
	});
	
	// If user search subscriber by name, then subscriber_details_by_name_form will be submitted.
	document.getElementById("subscriber_details_by_name_form").addEventListener('submit', function(e){
		e.preventDefault();

		// This variable is used to store the validate_form function result.
		var form_submit_result;
		
		// If form_submit_result will be true, then subscriber_details_by_name_form will be submitted, else not.
		form_submit_result = validate_form([subscriber_name]);
		if(form_submit_result === true){
			document.getElementById("subscriber_details_by_name_form").submit();
		}
	});
	...
	// This will make an ajax request to get subscriber name according to keyup event on the serach subscriber details form's subscriber name column.
	document.getElementById('subscriber_name').addEventListener('keyup', function(e){
		e.preventDefault();		
		// This variable is used to store the subscriber name, filled on the serach subscriber details form's subscriber name column.
		let details_subscriber_name = document.getElementById('subscriber_name').value;
		...
		xhr.send("subscriber_name=" + details_subscriber_name);
			...
			if(xhr.readyState == 4){
				if((xhr.status >= 200 && xhr.status < 300) || xhr.status == 304){
					if(xhr.responseText){						
						...
						document.getElementById('details_subscriber_name_list').innerHTML = "";
						...
						if(newResponse.not_found === undefined){
							for(let i=0; i<newResponse.length; i++){
								...
								document.getElementById('details_subscriber_name_list').appendChild(new_option);
							}
						}
						else{
